...
//数据缓冲区		  
unsigned char jpg_buffer[1028];//数据缓存区	
/////////////////////////////////////////////////////    
//初始化智能画点
void AI_Drow_Init(void)
{
	...
	if(temp1>1)temp1=1;	  
	//使图片处于所给区域的中间
	PICINFO.ImgXOff=(PICINFO.S_Width-temp1*PICINFO.ImgWidth)/2;
	PICINFO.ImgYOff=(PICINFO.S_Height-temp1*PICINFO.ImgHeight)/2;
	PICINFO.S_XOFF+=PICINFO.ImgXOff;
	PICINFO.S_YOFF+=PICINFO.ImgYOff;
...
{	
	int	funcret;//返回值	
	//得到显示方框大小	  	 
	if(ey>sy)PICINFO.S_Height=ey-sy+1;
	else PICINFO.S_Height=sy-ey+1;	 
	if(ex>sx)PICINFO.S_Width=ex-sx+1;
	else PICINFO.S_Width=sx-ex+1;
	//显示区域无效
...
{    		    	   
	...
	u32 count, uiTemp;
	u16 r, g, b;
	u16 *color_arry = (u16 *)iclip;
	u16 color_arryOff, rownum, pixelnum;
	u32 x ,y;  				   
	BITMAPINFO *pbmp;//临时指针 
	CurFile=BmpFileName; 
	F_Open(CurFile);//打开文件	 						  
	F_Read(CurFile,jpg_buffer);//读出512个字节
	F_Read(CurFile,jpg_buffer+512);//读出512个字节	  
	...
	x=PICINFO.ImgWidth;
	y=PICINFO.ImgHeight;     
	PICINFO.staticy=PICINFO.S_Height-(PICINFO.ImgYOff<<1);
	if(PICINFO.staticy>PICINFO.ImgHeight) PICINFO.staticy=PICINFO.ImgHeight;
	pixelnum = PICINFO.S_Width-(PICINFO.ImgXOff<<1);	//实际显示的一行中像素个数
	if(pixelnum>PICINFO.ImgWidth) pixelnum=PICINFO.ImgWidth;
	PICINFO.staticx=pixelnum;
	rownum = 2048/pixelnum;			   //4KBRAM空间可存放的显示行数
	color_arryOff = rownum*pixelnum-1;
	while(1)
	{				 
		if(color_byte==3)
		{
			r = jpg_buffer[count+2]&0xF8;
			g = jpg_buffer[count+1]&0xFC;
			b = jpg_buffer[count]&0xF8;
			b >>= 3;g <<= 3;r <<= 8;
			color_arry[color_arryOff] = b|g|r;
		}
		...
		{
			color_arry[color_arryOff] = *(u16 *)(&(jpg_buffer[count]));
//			r = color_arry[color_arryOff]&0xF800;
//			g = color_arry[color_arryOff]&0x07E0;
//			b = color_arry[color_arryOff]&0x001F;
//			r >>= 11;b <<= 11;
//			color_arry[color_arryOff] = b|g|r;
		}
		else
		...
		do
		{
			...
			if(x==0)
			{
				count += (uiTemp-PICINFO.ImgWidth*color_byte);
				x=PICINFO.ImgWidth;
				y--;
				if(color_arryOff==0||PICINFO.staticy==1)
				{
					if(PICINFO.staticy==1) 	   
					rownum = (PICINFO.S_Height-(PICINFO.ImgYOff<<1))%rownum;
					LCD_WriteCom(0x0036);LCD_WriteData(0x004a);	
					LCD_SetDisplayWindow(PICINFO.S_XOFF, \
										 PICINFO.staticy+PICINFO.S_YOFF-1, \
										 PICINFO.S_XOFF+pixelnum-1, \
										 PICINFO.staticy+PICINFO.S_YOFF+rownum-1);
					DMA_M2MStart((u32)&color_arry[color_arryOff], LCD_BASE+2, pixelnum*rownum);
					LCD_WriteCom(0x0036);LCD_WriteData(0x000a);
					if(PICINFO.staticy==1) return TRUE;
					else
					color_arryOff = rownum*pixelnum;
				}
				while(y*PICINFO.Div_Fac/10000 == PICINFO.staticy)
				{
					if(--y)
					count +=uiTemp;
					else return TRUE;	
				}
				PICINFO.staticy = y*PICINFO.Div_Fac/10000;					 
			}	 					 			   			
		}while(x*PICINFO.Div_Fac/10000 == PICINFO.staticx);
		PICINFO.staticx = x*PICINFO.Div_Fac/10000; 

		color_arryOff--;
				
		if(count<=1024-color_byte)
		{}		
		else if(count<1024)
		{
			F_Read(CurFile,jpg_buffer);
			for(i=1024-count,j=0;i<color_byte;i++,j++)
			jpg_buffer[1024+j] = jpg_buffer[j];
			mark = 0;
		}
		else
		{
			if(count-1024<1024)
			{
				if(mark&&(count-1024<512))	
				F_Read(CurFile,jpg_buffer);
				else mark = 1;
				F_Read(CurFile,jpg_buffer+512);
				count -= 1024;
			}
			else
		    {
				CurFile->F_Offset += (count-1023)/512;
				j=CurFile->F_Offset/SectorsPerClust;
				for(i=0;i<j;i++)
				CurFile->F_CurClust = FAT_NextCluster(CurFile->F_CurClust);
				CurFile->F_Offset %= SectorsPerClust;
				count = (count-1024)%512;
				F_Read(CurFile,jpg_buffer);//读出512个字节
				F_Read(CurFile,jpg_buffer+512);//读出512个字节	 
			}	 	 	
		} 	 
