...
u32 FileType_Tell(u8 * exName)
{
	...
	if(i==32) return 0x80000000;
	else return 1<<i;//返回文件类型
...
{  		   
	...
	DWORD hidsec=0;
	u32 Capacity;
	if(SDCard_Config() != SD_OK) return 0xFF;	 
	Capacity = SDCardInfo.CardCapacity; /* Card Capacity */				   
	if(Capacity<0xff) return 1;	    
	if(SD_ReadBlock(0,(u32 *)fat_buffer,512) != SD_OK) return 2;
	...
	{
		if(SD_ReadBlock((pr->prStartLBA)<<9,(u32 *)fat_buffer,512) != SD_OK)
		return 3;						//read the bpb sector
		bs = (bootsector710 *)fat_buffer;
		if(bs->bsJump[0]!=0xE9 && bs->bsJump[0]!=0xEB)
		{
			hidsec = 0;
			if(SD_ReadBlock(0,(u32 *)fat_buffer,512) != SD_OK) return 4;//read the bpb sector
			bs = (bootsector710 *)fat_buffer;	
		}
	}	  
...
RESN:
	...
	{		 
		...
		{														  
			...
			if(FAT_TAB.Fat_Head_Pos<Fat_Head_Size)//不超过范围
			{		  
			...
			else return FAT_NextCluster(cluster);
		} 
...
unsigned long FAT_NextCluster(unsigned long cluster)
{
	...
	if(SD_ReadBlock(sector<<9,(u32 *)fat_buffer,512) != SD_OK) return 0x0ffffff8;//读取FAT表,发生错误是返回0x0ffffff8
	if(FAT32_Enable)
	{
...
{ 			  	   
	...
	{			 
		...
		{
			if(SD_ReadBlock((FirstDirSector+cnt)<<9,(u32 *)fat_buffer,512) != SD_OK)
			return 0;				//读数错误    	   
	...
	else//其他文件夹/FAT32系统
	{
		...
		while(1)
		{
			...
			{
				if(SD_ReadBlock((sector+cnt)<<9,(u32 *)fat_buffer,512) != SD_OK)
				return 0;
				for(offset=0;offset<512;offset+=32)
				{
...
{	
	...
	if(SD_ReadBlock((sector+FileInfo->F_Offset)<<9,(u32 *)buf,512) != SD_OK)
	return 0;					//读数错误   
	FileInfo->F_Offset++;
