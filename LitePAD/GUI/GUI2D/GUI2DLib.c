...
static s16 DrawPosX;
static s16 DrawPosY;
u8 LineStyle=GUI_LS_SOLID;
u8 PenShape=GUI_PS_ROUND;
u8 PenSize=1;
/*********************************************************************
*
*       _SwapInt
...
*       _CalcOrto
*/
static void _CalcOrto(int xDiff, int yDiff, s32 r, int*px, int*py) {
  s32 x,y;
/*
...
*/
  ...
  int Angle = _atan2(xDiff, yDiff);
  Angle+= GUI_90DEG;
  x = ( r*(s32)GUI_cos(Angle));
  y = ( r*(s32)GUI_sin(Angle));
#endif
...
*       GL_DrawLine1
*/
static void GL_DrawLine1(int x0, int y0, int x1, int y1) {
  int xdiff = x1-x0;
  int ydiff = y1-y0;
  int xdiffby2;
  int i;
...
/* Draw pixel by pixel solid*/
  if (LineStyle == GUI_LS_SOLID) {
    for (i=0; i<=xdiff; i++) {
      s32 l = ((s32)ydiff)*i+xdiffby2;
      int y = (ABS(l)<32767) ? (y0 + ((int)l)/xdiff) : (y0 + l/xdiff);
      if (!Swapped)
        LCD_DrawPoint(x0+i, y);
      else
        LCD_DrawPoint(y, x0+i);
    }
...
void GL_DrawPoint(int x, int y) {
  if (PenSize == 1) {
    ...
    LCD_FillCircle(x, y, (PenSize - 1) / 2);
  }
}
...
void GL_DrawLine(int x0, int y0, int x1, int y1) {
  if (PenSize ==1) {
    ...
    if (xdiff | ydiff) {    
      ...
      _CalcOrto(x0-x1,y0-y1, (s32)(PenSize-1), &xOff, &yOff);
...
void GL_DrawLineRelNM(int dx, int dy) {
  GL_DrawLine(DrawPosX, DrawPosY, DrawPosX+dx, DrawPosY+dy);                
}
...
void GL_DrawLineTo(int x, int y) {
  GL_DrawLine(DrawPosX, DrawPosY, x, y); 
  DrawPosX = x;
  DrawPosY = y;
}
...
*       GUI_MoveRel
*/
void GL_MoveRel(int dx, int dy) { /*tbd: GL_LinePos. */
  DrawPosX += dx;
  DrawPosY += dy;
}
