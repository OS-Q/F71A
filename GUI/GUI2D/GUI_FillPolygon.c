...
static s16 _aX[GUI_FP_MAXCOUNT];
u16 *tempReshow;
u32 tempOff;
u16 PinAlpha;
u8 DrawFuc;
...
*		LCD_HL_DrawHLine
*/
extern unsigned char jpg_buffer[1028];
static void LCD_HL_DrawHLine(u16 sx, u16 y, u16 ex)
{
	u16 i,j;
	u16 *temp=(u16 *)jpg_buffer;
	if(SWAlpha)
	LCD_Fill(sx, y, ex, y, POINT_COLOR);
	else
	{
		j=ex-sx+1;
		if(DrawFuc)
		{
			LCD_ReadRect(sx, y, ex, y, tempReshow+tempOff);
			for(i=0;i<j;i++)
			temp[i]=AlphaColor(POINT_COLOR, *(tempReshow+tempOff+i), PinAlpha);
			LCD_WriteTempRect(sx, y, ex, y, temp);
		}
		else LCD_WriteTempRect(sx, y, ex, y, tempReshow+tempOff);
		tempOff += j;
	}
}
/*********************************************************************
...
void GL_DrawPolygonM(const GUI_POINT*pPoints, int NumPoints, int x0, int y0) {
  ...
  while (--NumPoints >0) {
    pPoint++;
    LCD_DrawLine((pPoint-1)->x+x0, (pPoint-1)->y+y0, pPoint->x+x0, pPoint->y+y0);
  }
  // Now draw closing line unless it has already been closed
  if ( (pPoint->x != pPoints->x)
     ||(pPoint->y != pPoints->y))
  {
    LCD_DrawLine(pPoint->x+x0, pPoint->y+y0, pPoints->x+x0, pPoints->y+y0);
  }
}
