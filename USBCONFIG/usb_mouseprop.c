...
#include "usb_lib.h"
#include "usb_conf.h"
#include "usb_mouseprop.h"
#include "usb_mousedesc.h"
#include "usb_pwr.h"
...
DEVICE Mouse_Device_Table =
  {
    EP_MouseNUM,
    1
  };

DEVICE_PROP Mouse_Device_Property =
  {
    Joystick_init,
    Joystick_Reset,
    Joystick_Status_In,
...
USER_STANDARD_REQUESTS Mouse_User_Standard_Requests =
  {
...
ONE_DESCRIPTOR Mouse_Device_Descriptor =
  {
    (u8*)Joystick_DeviceDescriptor,
    JOYSTICK_SIZ_DEVICE_DESC
  };

ONE_DESCRIPTOR Mouse_Config_Descriptor =
  {
...
ONE_DESCRIPTOR Mouse_String_Descriptor[4] =
  {
...
void Joystick_Reset(void)
{
  ...
  SetEPRxCount(ENDP0, Mouse_Device_Property.MaxPacketSize);
  SetEPRxValid(ENDP0);
...
{
  return Standard_GetDescriptorData(Length, &Mouse_Device_Descriptor);
}
...
{
  return Standard_GetDescriptorData(Length, &Mouse_Config_Descriptor);
}
...
u8 *Joystick_GetStringDescriptor(u16 Length)
{
  ...
  {
    return Standard_GetDescriptorData(Length, &Mouse_String_Descriptor[wValue0]);
  }
}
